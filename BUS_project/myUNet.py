...
class UNet(nn.Module): # nn.Module class를 UNet에 상속한다.
    def __init__(self):
        ...
        def CBR(in_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=True):
            ...
            cbr = nn.Sequential(
                nn.Conv2d(in_channels=in_channels, out_channels=out_channels), # bias
        ...
        # Contracting path
        self.enc1_1 = CBR(in_channels=1, out_channels=64) # enc1_1 = ..가 아니라 self.enc1_1로 생성하는 이유. tab시 빨간줄
        ...
        # Expansive path
        self.dec5_1 = CBR(in_channels=1024, out_channels=1024) # channel

        self.unpool4 = nn.ConvTranspose2d(in_channels=1024, out_channels=512, kernel_size=2, stride=2, padding=0, bias=True)

        self.dec4_2 = CBR(in_channels=1024, out_channels=512)
        self.dec4_1 = CBR(in_channels=512, out_channels=512)

        self.unpool3 = nn.ConvTranspose2d(in_channels=512, out_channels=256, kernel_size=2, stride=2, padding=0, bias=True)

        self.dec3_2 = CBR(in_channels=512, out_channels=256)
        self.dec3_1 = CBR(in_channels=256, out_channels=256)

        self.unpool2 = nn.ConvTranspose2d(in_channels=256, out_channels=128, kernel_size=2, stride=2, padding=0, bias=True)

        self.dec2_2 = CBR(in_channels=256, out_channels=128)
        self.dec2_1 = CBR(in_channels=128, out_channels=128)

        self.unpool1 = nn.ConvTranspose2d(in_channels=128, out_channels=64, kernel_size=2, stride=2, padding=0, bias=True)

        self.dec1_2 = CBR(in_channels=128, out_channels=64)
        self.dec1_1 = CBR(in_channels=64, out_channels=64)
