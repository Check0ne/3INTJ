...
def create_scheduler(name, optimizer, warmup_epochs, epochs, min_lr):
    if name == 'poly_lr':
        lr_scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=functools.partial(poly_learning_rate, warm_up_epoch=warmup_epochs, start_decay_epoch=epochs/10, total_epoch=epochs, min_lr=min_lr))            
