'''
<수정해야 할 사항>
1. Inference 할 때 original image size로 crop해서 dice score 계산하기
<문제점>
1. Normal case를 잘 판단하지 못한다.
2. Black background를 tumor로 인식한다.
'''
import os
import numpy as np
    ...
    if (epoch + 1) % val_interval == 0:
        ...
        with torch.no_grad():
            ...
            for val_data in val_loader:
                ...
                dice_score = dice_metric(y_pred=val_outputs, y=val_labels) # batch size가 인자로 들어가야하는구나... 
                
                if np.isnan(dice_score[0, 0].item()): # dice_score[0, 0]은 channel=0 의 dice_score, dice_score[0, 1]은 channel=1의 dice_score
                    metric += 1
                else:
                    metric += dice_score[0, 0].item()
                    
            # aggregate the final mean dice result
            #metric = dice_metric.aggregate().item()
            metric = metric / len(val_loader)
...
with torch.no_grad():
    ...
    for test_data in test_loader:
        ...
        #saveimg.save(f"{dice_score[0, 0].item()}input.png")
        #saveimg = transform(test_outputs[0])
        #saveimg.save(f"{dice_score[0, 0].item()}output.png")
        arr = test_labels[0].cpu().numpy().astype('float32')
        arr = (arr*255).astype('uint8')
        #saveimg = PIL.Image.fromarray(arr[0].squeeze(), mode = 'L')
        #saveimg.save(f"{dice_score[0, 0].item()}label.png")
        
        if np.isnan(dice_score[0, 0].item()):
            metric += 1
        else:
            metric += dice_score[0, 0].item()
            
    # aggregate the final mean dice result
    print("evaluation metric:", metric/len(test_loader))
    # reset the status
    dice_metric.reset()
#%%
